...
import sqlite3, json, httpx, os, uuid
...
GEMINI_API_KEY = "AIzaSyB_fMrQa2LtbZ1iU8z_2_XLAnAgSJ2YWNU"
...
async def chat_page(request: Request):
    ...
    with get_db() as db:
        ...
        history.reverse()
    return templates.TemplateResponse("chat.html", {"request": request, "user": user, "history": json.dumps(history)})
...
async def ai_chat(data: ChatMessage, request: Request):
    ...
    with get_db() as db:
        ...
        history = [dict(h) for h in db.execute("SELECT role,content FROM chat_history WHERE user_id=? ORDER BY created_at DESC LIMIT 10", (user["id"],)).fetchall()]
        history.reverse()
    ...
    system = f"""You are NEXUS, an AI life manager assistant for {u['name']}. Be helpful, warm, and concise.

Current context:
- Date: {datetime.now().strftime('%A, %B %d, %Y')}
- Plan: {u['plan'].title()}
- Today's events: {json.dumps(events)}
- Active tasks: {json.dumps(tasks)}
- Habits: {json.dumps(habits)}
- Goals: {json.dumps(goals)}

Keep responses under 200 words. Be actionable and encouraging."""

    messages = history[-6:] if history else []
    messages.append({"role": "user", "parts": [{"text": data.message}]})

    payload = {
        "system_instruction": {"parts": [{"text": system}]},
        "contents": [{"role": m["role"] if "role" in m else "user", "parts": [{"text": m.get("content", m.get("parts",[{}])[0].get("text",""))}]} for m in messages],
        "generationConfig": {"temperature": 0.8, "maxOutputTokens": 512}
    }
    # Fix: rebuild properly
    payload["contents"] = []
    for m in history[-6:]:
        payload["contents"].append({"role": m["role"], "parts": [{"text": m["content"]}]})
    payload["contents"].append({"role": "user", "parts": [{"text": f"{system}\n\nUser: {data.message}"}]})

    async with httpx.AsyncClient(timeout=30) as client:
        resp = await client.post(GEMINI_URL, json=payload)
        result = resp.json()
    ai_text = result.get("candidates", [{}])[0].get("content", {}).get("parts", [{}])[0].get("text", "Sorry, I couldn't process that.")
